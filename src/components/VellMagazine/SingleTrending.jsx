  ...
  return (
    <div className='flex lg:flex-col items-center justify-between mx-3 my-3 lg:my-0 gap-3'>
        ...
        <div className='w-7/12 sm:w-full mt-3 ml-3 lg:ml-0 '>
          <p className='text-[12px] font-bold hover:underline cursor-pointer font-serif'
        ...
        <p className='font-500  text-[13px] xl:py-3 line-clamp-3 hidden lg:block font-serif'>{content.startsWith("<p") ? content.slice(17,87).replace(/<.?p[^>]*>/g,"").replace(/<.?h.?[^>]*>/g,"") : content.slice(4,75).replace(/<.?p[^>]*>/g,"").replace(/<.?h.?[^>]*>/g,"")}... </p>
        <p className='font-[500] text-[13px] font-serif'>{changeFormat(date)}</p>
        </div>
    </div>
  )
}
