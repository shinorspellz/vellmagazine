...
const navigate=useNavigate()
    ...
    const relevantTopics=["MENTAL HEALTH","WEEKEND GETAWAYS","MAKEUP","PET INFLUENCERS","SOCIAL MEDIA HACKS","COST OF LIVING"]
...
console.log(item)
  ...
  return (
    ...
    <div className='max-w-[440px] flex flex-col '>
        ...
        <img src={img} alt="single-magazine"className='h-[131px] sm:h-auto mb-2 cursor-pointer  hover:scale-105' 
        onClick={(e)=>goToArticle(e)}
        />
        <p className='font-500 font-serif text-[14px] xl:py-3 line-clamp-1 xl:line-clamp-none'>{content.startsWith("<p") ? content.slice(17,200).replace(/<.?p[^>]*>/g,"").replace(/<.?h.?[^>]*>/g,"") : content.slice(4,200).replace(/<.?p[^>]*>/g,"").replace(/<.?h.?[^>]*>/g,"")}...</p>
        <p className='font-500 text-[14px] hidden xl:block text-left' >{changeFormat(date)}</p> */}
    </div>
  )
}
