  ...
  return (
    <div
      ...
      >
        ...
        />
        <div className="block absolute left-0 top-0 w-full h-full text-center text-2xl font-bold  pt-3 px-1 "
        onClick={()=>navigate(`/author/${item.name}`,{state:item.name})}     >
        <h3 className= "text-[#503C3B] authorName absolute bottom-0 left-0 w-full text-center bg-white/40">{item.name}</h3>
